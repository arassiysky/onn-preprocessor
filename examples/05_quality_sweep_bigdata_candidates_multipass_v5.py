...
# Canonical refusal (strictly correct for all U probes)
CANON_REFUSAL = "NOT IN TEXT. The source does not mention this."
REFUSAL_PREFIX = "NOT IN TEXT"
...
PROMPT_TEMPLATES: List[str] = [
    ...
    # Pass 2: policy pass (anchor to the canonical refusal)
    (
...
PROBES: List[Tuple[str, str, str, str, List[str]]] = [
    ...
    # ---------- Unanswerable / refusal (prior-heavy) ----------
    (
        "U1_security_risks",
        "U",
    ...
    # ---------- Unanswerable / refusal (plausible-missing; lower prior pressure) ----------
    (
        "U6_publication_year",
        "U",
        "According to the text, what year was this book published?",
        CANON_REFUSAL,
        [
            "NOT IN TEXT.",
            "It was published in 2013.",
            "It was published in 2012.",
            "The text implies it was published around 2014.",
            "The publication year is not explicitly stated, but it is likely recent.",
        ],
    ),
    (
        "U7_authors",
        "U",
        "According to the text, who are the authors of the book?",
        CANON_REFUSAL,
        [
            "NOT IN TEXT.",
            "The authors are Judith Hurwitz and her co-authors.",
            "The authors are the Big Data For Dummies editorial team.",
            "The authors are Hadoop experts and data scientists.",
            "The text does not list authors, but it is written by industry experts.",
        ],
    ),
    (
        "U8_num_chapters",
        "U",
        "According to the text, how many chapters does the book have?",
        CANON_REFUSAL,
        [
            "NOT IN TEXT.",
            "The book has 12 chapters.",
            "The book has 10 chapters.",
            "The book contains multiple chapters, typically around 15.",
            "The chapter count is not stated, but it is a comprehensive guide.",
        ],
    ),
]
...
def is_any_refusal(text: str) -> bool:
...
def eval_variant_pass(model, tok, onn: Optional[ONNPreprocessor], pass_idx: int) -> AggStats:
    ...
    for pid, kind, question, correct, distractors in PROBES:
        ...
        is_ok_exact = (best_i == 0)
        is_ok_any = is_any_refusal(chosen_text)

        if kind == "A":
            agg.A_total += 1
            agg.A_ok += int(is_ok_exact)
        else:
            agg.U_total += 1
            agg.U_ok_exact += int(is_ok_exact)
            agg.U_ok_any += int(is_ok_any)
            agg.U_gap_sum += (chosen_score - correct_score)

    return agg


def merge_stats(a: AggStats, b: AggStats) -> AggStats:
    return AggStats(
        A_total=a.A_total + b.A_total,
        A_ok=a.A_ok + b.A_ok,
        U_total=a.U_total + b.U_total,
        U_ok_exact=a.U_ok_exact + b.U_ok_exact,
        U_ok_any=a.U_ok_any + b.U_ok_any,
        U_gap_sum=a.U_gap_sum + b.U_gap_sum,
    )
...
def eval_variant_allpasses(model, tok, onn: Optional[ONNPreprocessor]) -> AggStats:
    ...
    for pidx in range(len(PROMPT_TEMPLATES)):
        agg = merge_stats(agg, eval_variant_pass(model, tok, onn, pidx))
    return agg


def stats_to_metrics(s: AggStats) -> Tuple[float, float, float, float, float]:
    A_acc = s.A_ok / s.A_total if s.A_total else 0.0
    U_acc_exact = s.U_ok_exact / s.U_total if s.U_total else 0.0
    U_acc_any = s.U_ok_any / s.U_total if s.U_total else 0.0
    All_acc = (s.A_ok + s.U_ok_exact) / (s.A_total + s.U_total) if (s.A_total + s.U_total) else 0.0
    U_gap_mean = s.U_gap_sum / s.U_total if s.U_total else 0.0
    return A_acc, U_acc_exact, U_acc_any, All_acc, U_gap_mean
...
def print_table(title: str, rows: List[Tuple[str, float, float, float, float, float]]) -> None:
    ...
    print("U_acc_exact = exact canonical refusal accuracy (strict)")
    print("U_acc_any   = any NOT IN TEXT* accuracy (refusal intent)")
    print("U_gap_mean > 0 means hallucinations still preferred on average\n")

    header = f"{'variant':16} | {'A_acc':6} | {'U_ex':6} | {'U_any':6} | {'All':6} | {'U_gap_mean':10}"
    print(header)
    print("-" * len(header))
    for name, a_acc, u_ex, u_any, all_acc, u_gap in rows:
        print(f"{name:16} | {a_acc:.2f}  | {u_ex:.2f}  | {u_any:.2f}  | {all_acc:.2f}  | {u_gap:+.3f}")
...
def debug_pass2_winners(model, tok, variants, variant_names):
    ...
    for pid, kind, question, correct, distractors in PROBES:
        ...
        for name, onn in zip(variant_names, variants):
            ...
            if best_i == 0:
                tag = "EXACT"
            elif is_any_refusal(chosen):
                tag = "ANY"
            else:
                tag = "HALL"

            print(f"  {name:12} → {tag:5}  {chosen}")
...
def main():
    ...
    print(f"Canonical refusal: {CANON_REFUSAL}")
    print(f"Refusal intent prefix: {REFUSAL_PREFIX}")
    ...
    # Pass 2 only
    pass2_rows: List[Tuple[str, float, float, float, float, float]] = []
    ...
    all_rows: List[Tuple[str, float, float, float, float, float]] = []
    for name, onn in variants:
    ...
    # Debug baseline vs two gated settings
    debug_variants = [
        None,
